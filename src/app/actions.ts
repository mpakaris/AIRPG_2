...
async function handleConversation(state: PlayerState, playerInput: string, game: Game): Promise<CommandResult> {
    ...
    // Ensure the NPC has canned responses before calling the AI
    ...
    if (!selectedResponse) {
        // Fallback to default if the AI returns an unexpected topic
        const defaultResponse = npc.cannedResponses.find(r => r.topic === 'default');
        const messageContent = defaultResponse?.response ?? "I don't know what to say.";
        return {
            newState: state,
            messages: [createMessage(npc.id as NpcId, npc.name, `"${messageContent}"`)]
        };
    }
    ...
    // Prepend the NPC's own message to the messages generated by the actions
