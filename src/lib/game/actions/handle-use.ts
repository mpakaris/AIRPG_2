...
export async function handleUse(state: PlayerState, itemName: string, targetName: string, game: Game): Promise<CommandResult> {
  ...
  if (normalizedTargetName) {
    ...
    const targetObjectId = location.objects.find(objId => normalizeName(game.gameObjects[objId]?.name).includes(normalizedTargetName));
  ...
  // Check for a simple onUse handler (not an array)
  ...
  // Fallback if the item exists but has no direct onUse handler or conditions aren't met
